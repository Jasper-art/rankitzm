...
export default function EnterScoresByLearnerScreen() {
  ...
  // Auto-select first learner on load
  useEffect(() => {
  ...
  const handleScoreChange = (subjectId: number, value: string) => {
    ...
    if (!isNaN(numValue) && numValue >= 0 && numValue <= maxMark) {
      ...
      showNotification(
        "error",
        `Score cannot exceed ${maxMark} for ${subject?.subjectName}`,
      );
    }
  };
  ...
  const handleSubmit = async (autoNext: boolean = true) => {
    ...
    const hasScores = classSubjects.some((subject) => scores[subject.id!]);
    if (!hasScores) {
    ...
    try {
      ...
      for (const subject of classSubjects) {
        ...
        if (score && score.trim()) {
          ...
          if (isNaN(scoreValue) || scoreValue < 0 || scoreValue > maxMark) {
            showNotification(
              "error",
              `Invalid score for ${subject.subjectName}. Must be between 0 and ${maxMark}.`,
            );
            setLoading(false);
            return;
          }
      ...
      showNotification(
        "success",
        `Recorded ${savedCount} score(s) for ${selectedLearnerInfo?.name}`,
      );

      await addActivity({
        type: "score_entered",
        title: "Scores entered",
        subtitle: `${selectedLearnerInfo?.name} • ${classInfo?.className}`,
        timestamp: Date.now(),
      ...
      if (autoNext) {
        const currentIndex = filteredLearners.findIndex(
          (l) => l.id === selectedLearner,
        );
        if (currentIndex < filteredLearners.length - 1) {
          setSelectedLearner(filteredLearners[currentIndex + 1].id!);
          setTimeout(() => {
            firstInputRef.current?.focus();
          }, 50);
        } else {
          showNotification("success", "All visible students completed!");
        }
      }
    } catch (err) {
      showNotification(
  ...
  const entriesCount = classSubjects.filter((s) => scores[s.id!]).length;
  ...
  return (
    ...
    >
      ...
      >
        {/* Left Sidebar: Learner List - Hidden on mobile unless menu is open */}
              ...
              {filteredLearners.length > 0 ? (
                filteredLearners.map((learner) => (
                  <button
                    key={learner.id}
                    onClick={() => {
                      setSelectedLearner(learner.id || null);
                      setScores({});
                    ...
                    style={{
                      ...
                      padding: "12px 20px",
                      border: "none",
                      borderLeft: `3px solid ${selectedLearner === learner.id ? t.accent : "transparent"}`,
                      background:
                    ...
                    <div>
                      <div
                        style={{
                          fontSize: 14,
                          fontWeight:
                            selectedLearner === learner.id ? 600 : 500,
                          color:
                            selectedLearner === learner.id
                              ? t.accentDark
                              : t.text,
              ...
              >
                {!isMobileView && (
                  ...
                  >
                    ...
                    to move quickly between inputs.
                  </div>
                )}
...
interface ScoreTableRowProps {
  ...
  onScoreChange: (subjectId: number, value: string) => void;
...
function ScoreTableRow({
  ...
  const maxMark = subject.maxMark || 100;
  const scoreValue = scores[subject.id!];
      ...
      <td style={{ padding: "12px 20px" }}>
        <div style={{ position: "relative", maxWidth: 120 }}>
          <input
            ...
            max={maxMark}
            value={scoreValue || ""}
            onChange={(e) => onScoreChange(subject.id!, e.target.value)}
            placeholder="—"
            style={{
              width: "100%",
              padding: "8px 12px",
